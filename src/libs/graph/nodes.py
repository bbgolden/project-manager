...
def assign_workflow(state: OverallState, config: RunnableConfig) -> Command[Literal["supervisor", "input_helper"]]:
    ...
    system_prompt = SystemMessage(
        ...
        Do not explicitly ask the user for which tools they would like to use. It is your job to inference that information from their messages.
    ...
    return Command(
        ...
        }, goto="input_helper" if len(response.followup) > 30 else "supervisor"
    )
...
def clarify_input(state: OverallState) -> Command[Literal["liaison", "project_maker_check", "task_maker_check", "resource_manager_check"]]:
...
def create_project_check(state: OverallState, config: RunnableConfig) -> Command[Literal["project_maker", "input_helper"]]:
    ...
    return Command(
        update={
            ...
            "followup": response.followup,
        }, goto="input_helper" if len(response.followup) > 30 else "project_maker"
    )
...
def create_task_check(state: OverallState, config: RunnableConfig) -> Command[Literal["task_maker", "input_helper"]]:
    system_prompt = SystemMessage(
        """
    ...
    return Command(
        update={
            "redirect": "task_maker_check",
            "followup": response.followup,
        }, goto="input_helper" if len(response.followup) > 30 else "task_maker"
    )

def create_task(state: OverallState, config: RunnableConfig) -> OverallState:
    system_prompt = SystemMessage(
        f"""
    ...
    return {"messages": [response]} if response.tool_calls else {"messages": [response], "output": "Ran create_task"}

def create_task_tools():
    return ToolNode(task_maker_tools)

def manage_resources_check(state: OverallState, config: RunnableConfig) -> Command[Literal["resource_manager", "input_helper"]]:
    system_prompt = SystemMessage(
        """
    ...
    return Command(
        update={
            "redirect": "resource_manager_check",
            "followup": response.followup,
        }, goto="input_helper" if len(response.followup) > 30 else "resource_manager"
    )

def manage_resources(state: OverallState, config: RunnableConfig) -> OverallState:
    system_prompt = SystemMessage(
        """
        You are helping to manage the resources assigned to specific tasks within a project.
        Resources are defined as named persons (with a required first and optional last name) that contribute their work to said tasks.
    ...
    return {"messages": [response]} if response.tool_calls else {"messages": [response], "output": "resource manager"}
